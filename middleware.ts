...
export async function middleware(req:NextRequest){
  ...
  // İlk kurulumda kullanıcıyı kilitlememek için env değişkenleri tamamlanana kadar koruma devreye girmez.
  if(!secret||!adminEmail||!adminPassword) return NextResponse.next();

  const ok=await valid(req.cookies.get(COOKIE)?.value,secret);
  if(ok){
    if(pathname==="/giris") return NextResponse.redirect(new URL("/",req.url));
