...
export default async function OrderDetailPage({params}:{params:Promise<{id:string}>}){
  ...
  const [order]=await sql`SELECT id,marketplace_order_number,status,gross_amount,ordered_at,updated_at FROM orders WHERE id=${id} LIMIT 1`;
  if(!order) notFound();
  const items=await sql`SELECT product_name,barcode,quantity,unit_price,cost_at_sale FROM order_items WHERE order_id=${id} ORDER BY id`;
  const finance=await sql`SELECT type,amount,commission_amount,seller_revenue,transaction_at FROM financial_transactions WHERE order_id=${id} ORDER BY transaction_at DESC`;
  ...
  return <main className="content">
    ...
    <section className="metricGrid">
      ...
      <article className="card metricCard"><p className="metricLabel">Net</p><strong className="metricValue">{hasCost?money(net):'Maliyet bekliyor'}</strong><p className="metricNote">Satış − maliyet − komisyon − kargo</p></article>
    </section>
    <section className="card ordersCard"><div className="cardHeader"><div><h2>Ürünler</h2><p className="muted">Siparişteki ürün satırları.</p></div></div><div className="tableWrap"><table><thead><tr><th>Ürün</th><th>Barkod</th><th>Adet</th><th>Birim Fiyat</th><th>Satış Maliyeti</th></tr></thead><tbody>{items.map((i:any,idx:number)=><tr key={idx}><td>{i.product_name}</td><td>{i.barcode||'-'}</td><td>{i.quantity}</td><td>{money(Number(i.unit_price))}</td><td>{Number(i.cost_at_sale)>0?money(Number(i.cost_at_sale)):'Eksik'}</td></tr>)}</tbody></table></div></section>
