...
export default async function Home(){
 ...
 const [p]:any=await sql`SELECT COUNT(*) FILTER (WHERE ordered_at>=date_trunc('day',now() AT TIME ZONE 'Europe/Istanbul'))::int today_orders,COALESCE(SUM(gross_amount) FILTER (WHERE ordered_at>=date_trunc('day',now() AT TIME ZONE 'Europe/Istanbul')),0)::float today_revenue,COUNT(*) FILTER (WHERE ordered_at>=now()-interval '7 days')::int week_orders,COALESCE(SUM(gross_amount) FILTER (WHERE ordered_at>=now()-interval '7 days'),0)::float week_revenue,COUNT(*) FILTER (WHERE ordered_at>=now()-interval '30 days')::int month_orders,COALESCE(SUM(gross_amount) FILTER (WHERE ordered_at>=now()-interval '30 days'),0)::float month_revenue FROM orders`;
 const [finance]:any=await sql`SELECT COALESCE(SUM(CASE WHEN type IN ('Sale','Return') THEN commission_amount ELSE 0 END),0)::float commission,COALESCE(SUM(CASE WHEN type='Cargo' THEN amount ELSE 0 END),0)::float cargo FROM financial_transactions WHERE transaction_at>=now()-interval '30 days'`;
 const [cost]:any=await sql`SELECT COALESCE(SUM(oi.quantity*p.cost),0)::float product_cost,COUNT(*) FILTER (WHERE COALESCE(p.cost,0)<=0)::int missing FROM order_items oi JOIN orders o ON o.id=oi.order_id LEFT JOIN products p ON p.id=oi.product_id WHERE o.ordered_at>=now()-interval '30 days'`;
 const daily:any[]=await sql`SELECT to_char((ordered_at AT TIME ZONE 'Europe/Istanbul')::date,'DD.MM') day,COUNT(*)::int orders,COALESCE(SUM(gross_amount),0)::float revenue FROM orders WHERE ordered_at>=now()-interval '7 days' GROUP BY (ordered_at AT TIME ZONE 'Europe/Istanbul')::date ORDER BY (ordered_at AT TIME ZONE 'Europe/Istanbul')::date`;
 const recent:any[]=await sql`SELECT o.marketplace_order_number,o.status,o.gross_amount,o.ordered_at,COALESCE(string_agg(oi.product_name,', '),'') products FROM orders o LEFT JOIN order_items oi ON oi.order_id=o.id GROUP BY o.id ORDER BY o.ordered_at DESC LIMIT 8`;
 ...
 return <main className="shell appleShell">
  <aside className="sidebar appleSidebar">
   ...
   <div className="sidebarFooter"><span className="statusDot"/><span><strong>Trendyol bağlı</strong><small>5 dakikada bir senkron</small></span></div>
  </aside>
  ...
  <section className="content appleContent">
   ...
   <section className="card ordersCard premiumTable">
    ...
    <div className="tableWrap"><table><thead><tr><th>Sipariş</th><th>Ürün</th><th>Satış</th><th>Tarih</th><th>Durum</th></tr></thead><tbody>{recent.map((o:any)=><tr key={o.marketplace_order_number}><td><strong>#{o.marketplace_order_number}</strong></td><td className="productCell">{o.products||"-"}</td><td>{moneyFull(Number(o.gross_amount))}</td><td>{new Date(o.ordered_at).toLocaleDateString("tr-TR",{timeZone:"Europe/Istanbul",day:"2-digit",month:"short",hour:"2-digit",minute:"2-digit"})}</td><td><span className="statusPill">{o.status}</span></td></tr>)}</tbody></table></div>
   </section>
  </section>
 </main>
}
